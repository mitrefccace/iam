...
ACE Direct Configuration assumes the following about OpenAM:

1. OpenAM is running behind NGINX.
1. OpenAM has been assigned both private and public IP addresses.
1. The keystore files (cert.pem, key.pem) have been acquired from a trusted certificate authority for the domain of the server (required to secure the login to OpenAM).
1. Root privileges are available to install the software.
1. A three-level fully-qualified domain name (FQDN) e.g. test.examples.com is available. Note that an underscore (_) in the FQDN is *not* supported (e.g. my_host.domain.com is invalid).
1. Verify that running the `hostname` command returns a FQDN that corresponds to the domain that aligns with the cert.pem and key.pem.
1. Update /etc/hosts with entries for ACE Direct node server.
1. If the OpenAM server is behind a network proxy, create an entry for the http_proxy for wget. Depending on your operating system, it may be in ~/.wgetrc or /etc/wgetrc.
...
1. Add the following lines in /etc/security/limits.conf assuming the username running OpenAM is "centos":
...
1. Recommended steps - install the required software, update the configuration parameters and use the Automated Installation (Option 1).
...
* The default configurations will likely work out of the box depending on your environment, but before attempting an automated install, all config.json parameters should be verified/set.
...
### Global Section in ~/iam/config.json
...
### Apache Tomcat Section in ~/iam/config.json

Note - all of the following configuration file paths are relative to: */iam/scripts/*
...
### OpenAM Section in ~/iam/config.json

* [Download](https://backstage.forgerock.com/downloads/search?q=openam%2013) the OpenAM 13.0 zip file and unzip.
...
### SSL Configuration in ~/iam/tomcat/server.xml
...
### ~/iam/config/oam/config.properties

Update the following values:

* **SERVER_URL**: (URL of the Tomcat Server. Make sure the port number matches server.xml)
  * **NOTE**: URL name should NOT contain _ and the FQDN is at least two levels deep.
...
1. Update OAM config.properties in iam-configs
...
1. Run the following commands in this order (without sudo if running as root):

    * Install Java if not installed `sudo python java_installer.py`
    * Generate the keystore: `python keystore.py`
    * Verify/add the following lines to ~/.bashrc:
    ...
    * Execute : `source ~/.bashrc` to verify that there aren't any errors.
   ...
   The following two steps create OpenAM users using a script. If you want to manually create users (via the OpenAM GUI), skip these two steps.

   * (Optional) Completely follow the [Administration Tools Setup](#administration-tools-setup) directions in the [Manual Installation](#manual-installation) section in the second half of this guide. This is necessary in order for the automated *create_users.py* script to work properly. Make sure that your newly created pwd.txt file path is correct in *config.json*
   * (Optional) Create your OpenAM users `sudo python create_users.py`

Verify OpenAM software is installed and configured by entering the following URL on the browser if it is not behind a proxy server: `https://<FQDN>:<port>/ace`  e.g. `https://some.fqdn.com:8443/ace`

The ACE login page should be displayed.

**To auto install the software interactively (with prompts for input):**

1. Execute the above commands without the silent flag (`-silent`).
1. Update all configuration properties when prompted.
   1. The files will be opened the vim editor. Save the file when you are finished editing (`Escape :wq`)
...
1. Verify that `openssl` is installed on your system. This is a requirement and should be installed on CentOS by default.
...
1. Move your keystore created with `keytool` to /opt/tomcat
...
2. Make sure you have followed the SSL Configuration section of [Configuration Parameters](#configuration-parameters) -> SSL Configuration using server.xml towards the beginning of this guide
...
1. Execute the following commands to copy relevant ace direct images to openam deployment directory
...
1. Update the config.properties file's parameters outlined in the **Configuration Parameters** section of this guide
...
A reference outlining a full OpenAM installation can be found [here](https://backstage.forgerock.com/docs/openam/13/install-guide/#chap-install-tools) for additional information.
...
1. Change directories to the location of the SSOAdminTools
...
1. To verify that the file was edited correctly: `$ tail -2 ssoadm` should show the following output:
...
1. Verify that the **ssoadmn** command works properly:
    ...
    * Create a text file, for example `iam/config/oam/SSOAdminTools-13.0.0/ace/bin/pwd.txt`, containing the OpenAM administrative user's password string in cleartext on a single line.
